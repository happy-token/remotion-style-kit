...
interface GenerateRequest {
  ...
  frameImages?: string[];
  /** Encoded style configuration for visual guidance */
  styleConfig?: string;
}
...
export async function POST(req: Request) {
  ...
  // Build style guidance from the encoded style config
  let styleGuidance = "";
  if (encodedStyleConfig) {
    const config = decodeStyleConfig(encodedStyleConfig);
    if (config) {
      styleGuidance = buildStylePrompt(config);
    }
  }
...
Analyze the request and decide: use targeted edits (type: "edit") for small changes, or full replacement (type: "full") for major restructuring.`;
      ...
      const followUpSystemPrompt = styleGuidance
