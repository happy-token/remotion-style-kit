...
import { decodeStyleConfig, VISUAL_STYLE_PRESETS } from "../../styles";
import type {
  AssistantMetadata,
  EditOperation,
...
function GeneratePageContent() {
  ...
  const encodedStyleConfig = searchParams.get("style") || undefined;

  const styleConfigParsed = encodedStyleConfig
    ? decodeStyleConfig(encodedStyleConfig)
    : null;
        ...
        <div className="flex-1 flex flex-col min-w-0 pr-12 pb-8 overflow-hidden">
          {styleConfigParsed && (
            <div className="flex items-center gap-2 pt-4 pb-2">
              ...
              <span className="text-xs font-medium text-foreground bg-muted/30 px-2 py-0.5 rounded-full">
                {VISUAL_STYLE_PRESETS[styleConfigParsed.visualStyle].name}
              </span>
              <span className="text-[10px] text-muted-foreground">/</span>
              <span className="text-xs text-muted-foreground capitalize">
                {styleConfigParsed.motionStyle}
              </span>
              <span className="text-[10px] text-muted-foreground">/</span>
              <span className="text-xs text-muted-foreground">
                {styleConfigParsed.format.aspectRatio}
              </span>
            </div>
          )}
          <TabPanel
